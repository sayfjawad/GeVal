...
public class EmailAccountLookupImpl implements EmailAccountLookup {
    ...
    void setFilterTemplate(String filterTemplate) {
        Assert.isTrue(StringUtils.contains(filterTemplate, EMAILADRES_PLACEHOLDER), "filter '" + filterTemplate + " moet de placeholder '" + EMAILADRES_PLACEHOLDER + "' bevatten.");
    ...
    public Optional<EmailAccount> findByEmailAdres(String emailAdres) {
        ...
        String filter = filterTemplate.replaceAll(EMAILADRES_PLACEHOLDER, escape(emailAdres));
        try {
            List<EmailAccount> result = ldapTemplate.search(
                    query().base(baseDn).filter(filter),
                    (AttributesMapper<EmailAccount>) attributes -> emailAccountMapper.mapFromAttributes(attributes));
