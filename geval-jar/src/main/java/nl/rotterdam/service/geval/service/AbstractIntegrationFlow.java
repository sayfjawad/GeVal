...
public abstract class AbstractIntegrationFlow {
    ...
    /**
     * Log naam van processtap en voeg deze toe als header.
     * Log tevens de message payload indien zodanig ingeschakeld.
     *
     * @param procesStap
     * @return
     */
    ...
    /**
     * Log naam van processtap en voeg deze toe als header.
     * Roep bovendien een routine aan om additionele logging te realiseren.
     *
     * @param procesStap
     * @param messageLogger   consumer of Message for logging purposes
     * @return
     */
    protected Consumer<HeaderEnricherSpec> log(final String procesStap, final Consumer<Message> messageLogger) {
        return h -> h.header("procesStap", new HeaderValueMessageProcessor<String>() {
            ...
            public String processMessage(Message message) {
                ...
                final String e2euuid = requestContext != null ? requestContext.getId() : null;

                Function<Object, String> stringify = o -> o == null ? null : o.toString();
                Object url = stringify.apply(message.getHeaders().get("http_requestUrl"));
                String timestamp = stringify.apply(message.getHeaders().get("timestamp"));
