...
/**
 * Verantwoordelijk voor het valideren van e-mailadressen.
 * Let op! Voor de validatie wordt GEEN gebruik gemaakt van SMTP zelf (o.a. omdat vanwege de netwerkinrichting
 * de SMTP communicatie van deze service zal worden afgewezen door andere SMTP servers)
 */
@Component
public class EmailValidator implements Validator {
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(EmailValidator.class);
    private static final String MONITOR_ID = "email_validator";
    private static final String MONITOR_NAME = "Email-validator";
    ...
    /**
     * Status aanduidingen van een (potentieel) e-mailadres in oplopende mate van kennisvergaring/geavanceerdheid
     */
    private enum EmailStatus {
        ...
        /**
         * Er is een gebrek aan tijd (d.w.z. een time-out is aanstaande) waardoor niet de volledige controle is uitgevoerd.
         * Deze enum is bedoeld als een informatieve, ondersteunende status.
         */
        TIJDGEBREK(null);
    ...
    /**
     * Lookup voor e-mailadressen van de eigen organisatie.
     * Het gebruik van de lookup is optioneel
     */
    ...
    public CheckResultaat valideer(String emailAdres, int tijdsbestek) {
        ...
        final List<EmailStatus> status = new ArrayList<>();
        final String domain = getDomainName(emailAdres);
        if (domain == null) {
    ...
    /**
     * Voer een check uit met inachtneming van tijdsbestek. Indien de check niet op tijd
     * is uitgevoerd, dient het alternatief te worden uitgevoerd.
     *
    ...
    boolean checkDomainHasMX(final String domainName) {
        try {
            final Record[] records = new Lookup(domainName, Type.MX).run();
    ...
    String getDomainName(final String emailAddress) {
        final int offset = emailAddress.indexOf('@');
    ...
    String getUser(final String emailAddress) {
        ...
        final String domain = getDomainName(emailAddress);
        return domain == null ? null : emailAddress.substring(0, emailAddress.length()-domain.length()-1);
    }
    ...
    private boolean isMyOrg(final String adres) {
        final String lowerCaseAdres = adres.toLowerCase();
    ...
    /**
     * Controleer of het opgegeven e-mailadres van de eigen organisatie bestaat
     * door deze op te zoeken in de interne systemen (i.e. LDAP).
     *
     * @param emailAdres
     * @return
     */
    private boolean checkMyOrg(String emailAdres) {
     ...
     */
    private static class EmailSyntaxValidator extends org.apache.commons.validator.routines.EmailValidator {
            ...
            try {
                CompletableFuture.supplyAsync(() -> {
                    try {
                        ...
                        LOGGER.error("EmailAccountLookup wordt uitgeschakeld vanwege een verbindingsfout", cx);
