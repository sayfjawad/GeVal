 ...
 * andere bestanden worden vervangen door URLs zodat de bestanden via HTTP zijn op te vragen. Deze
 * bestanden staan op de classpath en de mogelijke padverwijzingen naar andere bestanden kunnen
 * zowel relatief als absoluut zijn. Ook is het mogelijk dat de padverwijzingen voorzien zijn van
 * een 'classpath:xsd/' prefix. In dat geval is de padverwijzing absoluut.
 */
public class WsdlSupplier {
     ...
     * @throws Exception
     */
    public String getWsdl(final String bestandspad, final String requestPath) throws Exception {

        if (isNotBlank(bestandspad)) {
            if (bestandspad.endsWith(".xsd") || bestandspad.endsWith(".wsdl")) {
                return processPayload(Paths.get(bestandspad),
                        readResourceContentAsString(bestandspad), requestPath);
            } else {
                throw new IllegalArgumentException(
                        "Ongeldig bestand wordt opgevraagd:" + bestandspad);
            }
        } else {
            //hoofd wsdl wordt opgevraagd
            return processPayload(Paths.get(mainWsdlFile),
                    readResourceContentAsString(mainWsdlFile), requestPath);
        }
    }

    private String processPayload(final Path currentFilePath, final String payload,
            final String requestPath) throws XPathExpressionException {

        final var doc = SoapUtil.parse(payload);
        final var xPath = getXpath();
        var importNodes = (NodeList) xPath.evaluate("//xs:import", doc, XPathConstants.NODESET);
        ...
        final var includeNodes = (NodeList) xPath.evaluate("//xs:include", doc,
                XPathConstants.NODESET);
        for (int i = 0; i < includeNodes.getLength(); i++) {
            processXsImportOrIncludeNode(includeNodes.item(i), currentFilePath, requestPath);
        }
        final var addressNode = (Node) xPath.evaluate("//soap:address", doc, XPathConstants.NODE);
        if (addressNode != null) {
            addressNode.getAttributes().getNamedItem("location")
                    .setNodeValue(requestPath + "/wsdl");
        }
        return SoapUtil.toString(doc);
    }

    private void processXsImportOrIncludeNode(final Node item, final Path currentFilePath,
            final String requestPath) {

        final String location = translateLocation(item, "schemaLocation", currentFilePath,
                requestPath);
        item.getAttributes().getNamedItem("schemaLocation").setNodeValue(location);
    }

    private void processWsdlImportNode(final Node item, final Path currentFilePath,
            final String requestPath) {
    ...
    private String translateLocation(final Node item, final String locationTag,
            final Path currentFilePath, final String requestPath) {
    ...
    public void setMainWsdlFile(final String mainWsdlFile) {

        this.mainWsdlFile = mainWsdlFile;
    }

    protected String readResourceContentAsString(final String fileName) throws Exception {

        final var inStream = this.getClass().getClassLoader().getResourceAsStream(fileName);
        try {
            return IOUtils.toString(inStream, StandardCharsets.UTF_8);
        } catch (Exception exc) {
            LOGGER.error(
                    "Onbekende fout opgetreden bij het ophalen van een wsdl of xsd: " + fileName,
                    exc);
            throw exc;
        }
    }
