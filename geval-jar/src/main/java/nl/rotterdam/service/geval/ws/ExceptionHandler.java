...
public class ExceptionHandler {
    ...
    public Message<String> handleException(ErrorMessage errorMessage) throws IOException {
        ...
        if (isJsonContent(errorMessage.getOriginalMessage())) {
            ...
            payload = SoapUtil.toString(SoapUtil.generateSoapFault((Document) result.getNode(),
                    Foutcodes.INVALIDE_INPUT_FOUT.equals(foutcode) ? SoapUtil.SoapFaultCodes.CLIENT : SoapUtil.SoapFaultCodes.SERVER,
                    foutMessage(cause)));
    ...
    private String getOriginalMessage(ErrorMessage errorMessage) {
        final Object payload = errorMessage.getOriginalMessage().getPayload();
    ...
    private boolean isJsonContent(Message<?> message) {
        final MediaType contentType = (MediaType)message.getHeaders().get(MessageHeaders.CONTENT_TYPE);
