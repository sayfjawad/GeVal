...
public class XsdValidator {
    ...
    public XsdValidator(final String... xsdResourcePath) throws SAXException {
        ...
        for (int i=0; i< xsdResourcePath.length; i++) {
            sources[i] = new StreamSource(IOUtils.getResourceAsUrl(xsdResourcePath[i], XsdValidator.class).toExternalForm());
        }
        this.validator = factory.newSchema(sources).newValidator();
    }

    public synchronized boolean validate(Message<?> message) throws MessagingException {
        try {
